class StudentCopy < Prawn::Document
  def initialize(students, gc_date)
    super(page_layout: :landscape, background: open('app/assets/images/logo-golden.jpg'))
    @students = students
    gc_date = Date.parse(gc_date)
    @students.each_with_index do |stud, _index|
      text 'Golden State College', inline_format: true, size: 12, align: :center, font_style: :bold
      move_down 10
