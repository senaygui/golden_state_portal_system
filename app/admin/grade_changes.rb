ActiveAdmin.register GradeChange do
      ...
      f.inputs 'Grade Change Request' do
        ...
        unless f.object.new_record?
          ...
          if (current_admin_user.role == 'instructor') || (current_admin_user.role == 'admin')
            f.input :add_mark, label: 'Mark Added (Total Assesment 100%)'
            f.input :instructor_reason
            f.input :instructor_approval, as: :select, collection: %w[pending approved denied],
                                          include_blank: false
    ...
    show title: proc { |grade_change|
                  truncate(
                    "#{grade_change.student.first_name.upcase} #{grade_change.student.middle_name.upcase} #{grade_change.student.last_name.upcase}", length: 50
                  )
                } do
      columns do
        column do
    ...
    batch_action :approve_instructor, if: proc {
      ...
      GradeChange.find(ids).each do |grade_change|
        grade_change.update(instructor_approval: 'approved', instructor_name: current_admin_user.name.full,
    ...
    batch_action :deny_instructor, if: proc {
      ...
      GradeChange.find(ids).each do |grade_change|
        grade_change.update(instructor_approval: 'denied', instructor_name: current_admin_user.name.full,
                            instructor_date_of_response: Time.zone.now)
      end
    ...
    batch_action :approve_department, if: proc {
      ...
      GradeChange.find(ids).each do |grade_change|
        grade_change.update(department_approval: 'approved', department_head_name: current_admin_user.name.full,
    ...
    batch_action :deny_department, if: proc {
      ...
      GradeChange.find(ids).each do |grade_change|
        grade_change.update(department_approval: 'denied', department_head_name: current_admin_user.name.full,
                            department_head_date_of_response: Time.zone.now)
      end
    ...
    batch_action :approve_dean, if: proc {
      ...
      GradeChange.find(ids).each do |grade_change|
        grade_change.update(dean_approval: 'approved', dean_name: current_admin_user.name.full,
    ...
    batch_action :deny_dean, if: proc {
      ...
      GradeChange.find(ids).each do |grade_change|
        grade_change.update(dean_approval: 'denied', dean_name: current_admin_user.name.full,
                            dean_date_of_response: Time.zone.now)
      end
    ...
    batch_action :approve_registrar, if: proc {
      ...
      GradeChange.find(ids).each do |grade_change|
        grade_change.update(registrar_approval: 'approved', registrar_name: current_admin_user.name.full,
    ...
    batch_action :deny_registrar, if: proc {
      ...
      GradeChange.find(ids).each do |grade_change|
        grade_change.update(registrar_approval: 'denied', registrar_name: current_admin_user.name.full,
                            registrar_date_of_response: Time.zone.now)
      end
