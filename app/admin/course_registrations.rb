ActiveAdmin.register CourseRegistration do
  ...
  scoped_collection_action :scoped_collection_update, title: 'Set Section', form: lambda {
  ...
  csv do
    ...
    column(:course_title)
    column('Program') { |c| c.program.program_name }
    column('Section') { |c| c.section.section_short_name if c.section.present? }
    column(:semester)
    column(:year)
    column(:enrollment_status)
    column('Created At') { |c| c.created_at.strftime('%b %d, %Y') }
  end
end
